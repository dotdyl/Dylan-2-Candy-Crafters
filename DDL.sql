...
-- Disabling commits and foreign key checks, and other initilzation statements for the dump
...
CREATE OR REPLACE TABLE `OrderDetails` (
  ...
  FOREIGN KEY (`candyId`) REFERENCES `Candies` (`candyId`) ON DELETE SET DEFAULT, -- We want to preseve order information even if a candy is deleted, so set default if deleted
